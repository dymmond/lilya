...
Let's explore the additional security components that Lilya offers out-of-the-box.

You don't need to implement them exactly as described here. Instead, you can use the special `Security` object from Lilya to inject security into each API.

Additionally, there are packages like `lilya-simple-jwt` that implement Bearer token checks at the middleware level, rather than using the `Security` object. This flexibility allows you to choose the approach that best suits your needs.
...
It is important to understand that Lilya only provides certain tools to help you with your job but you
