...
In the [previous chapter](./introduction.mdx), the security system—based on **Lilya's** dependency injection system was providing the `path operation function` with a `token` as a `str`.
...
By creating a `user` model you can use `Pydantic`, msgspec or whatever you want since Lilya supports the [encoders](../../encoders.mdx)
...
[dependency injection with Lilya](../../dependencies.mdx).
