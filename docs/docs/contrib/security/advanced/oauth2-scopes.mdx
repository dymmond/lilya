...
We'll quickly review the updates in the main [OAuth2 with Password, Bearer with JWT tokens](../oauth-jwt.mdx)` {.internal-link target=_blank} `, now enhanced with OAuth2 scopes:
