...
* You get them from the [request](./requests.mdx) object.
...
Since the `ChildLilya` is a representation of a [Lilya](./applications.mdx) class, we can pass
the otherwise limited parameters in the [custom router](#custom-router) and all the parameters available to
[Lilya](./applications.mdx).
...
include you can add specific unique [permissions](./permissions.mdx) and [middlewares](./middleware.mdx) which are available on each
...
* **permissions** - A list of [permissions](./permissions.mdx) to serve the application incoming
requests (HTTP and Websockets).
* **middleware** - A list of [middleware](./middleware.mdx)  to run for every request.
* **exception handlers** - A dictionary of [exception types](./exceptions.mdx) (or custom exceptions) and the handler
...
As specified before, the [middleware](./middleware.mdx) of a Path are read from top down,
from the parent to the very handler and the same is applied to [exception handlers](./exceptions.mdx),
and [permissions](./permissions.mdx).
...
Permissions are a must in **every** application. More on [permissions](./permissions.mdx) and how
to use them.
