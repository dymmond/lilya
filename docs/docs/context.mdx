...
Very similar approach to [Request](./requests.mdx) in terms of implementation but not the same.
...
The `context` also provides access to the [`request`](./requests.mdx) object as well as the
[application settings](./settings.mdx) and other functions.
...
The [settings](./settings.mdx) being used by the Lilya application.
...
:::danger
    When using the `request_context` object, you **must** install the [RequestContextMiddleware](./middleware.mdx#requestcontextmiddleware)
...
:::danger
    When using the `session` object, you **must** install the [SessionMiddleware](./middleware.mdx#sessioncontextmiddleware)
    and the [SessionContextMiddleware](./middleware.mdx#sessioncontextmiddleware) in this specific order as the order matters or an `ImproperlyConfigured` exception is raised.
:::
