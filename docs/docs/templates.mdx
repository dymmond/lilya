...
Note that internally the template response switches the render method and uses the [async content](./responses.mdx#async-content) feature
...
or any other mechanism for instantiating and validating submitted form data. See the [encoders](./encoders.mdx) for more
details.
