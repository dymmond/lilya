...
[Path](./routing.mdx#path) and [WebSocketPath](./routing.mdx#websocketpath) making sure that remains
as one source of truth and that also means that the [auto discovery](./routing.mdx#auto-discovering-the-parameters) of
