...
- [add_asgi_app()](./routing.mdx#add_asgi_app) is now supported and serves as an utility if you want to add any ASGI
...
- Controllers now allow you to pass "init" values without instantiating by calling the [`.with_init(...)`](./controllers.mdx#using-with_init).
...
- Support for [dependency overrides](./dependencies.mdx#dependency-overrides). This allows you to override dependencies globally in the application
...
- [Environments](./environments.mdx) documentation section with examples and usage.
...
- New [Lifecycle](./lifecycle.mdx) system allowing to create global and app level lifecycle hooks. With more explanatory
documentation.
...
- New security [SignedURL](./contrib/security/signed-urls.mdx) utility for generating and verifying time-limited signed URLs.
- New [SSEChannel](./contrib/sse.mdx) class for creating in-memory Server-Sent Events channels with pub/sub support.

### New Responses

- [CSVResponse](./responses.mdx#csvresponse) as a new available response for CSV files directly in the `lilya.responses`.
- [YAMLResponse](./responses.mdx#yamlresponse) as a new available response for YAML files directly in the `lilya.responses`.
- [XMLResponse](./responses.mdx#xmlresponse) as a new available response for XML files directly in the `lilya.responses`.
- [MessagePackResponse](./responses.mdx#messagepackresponse) as a new available response for MessagePack files directly in the `lilya.responses`.
- [NDJSONResponse](./responses.mdx#ndjsonresponse) as a new available response for Newline Delimited JSON files directly in the `lilya.responses`.
- [ImageResponse](./responses.mdx#imageresponse) as a new available response for images directly in the `lilya.responses`.
- `TextResponse` as a new available response for plain text files directly in the `lilya.responses`.
- [EventStreamResponse](./responses.mdx#eventstreamresponse) as a new available response for Server-Sent Events directly in the `lilya.responses`.
...
- **Async testing utilities**:
    - Introduced [AsyncTestClient](./test-client.mdx#the-async-test-client) and [create_async_client()](./test-client.mdx#context-manager-create_async_client) to allow fully asynchronous testing of Lilya apps.
...
- Contrib Shortcuts:
  - New [abort](./contrib/shortcuts/abort.mdx) function to raise HTTP exceptions.
  - New [responses shorcuts](./contrib/shortcuts/responses.mdx) including:
...
- [display_in_cli](./directives/directive-decorator.mdx#the-display_in_cli-option) to `@directive`.
...
- Wrong example for the CSRF token in the [security with CSRF](./contrib/security/csrf.mdx).

## 0.20.6

### Added

- [Relay](./contrib/proxy/relay.mdx). This allows to create objects that are ASGI compatible and upstream services within your Lilya application.
...
- Support for [CSRFMiddleware](./middleware.mdx#csrfmiddleware) to understand the HTML forms allowing also custom fields.
- New [HTML](./responses.mdx#html) response as an alternative to `HTMLResponse`.
- New documentation section for [security with CSRF](./contrib/security/csrf.mdx).
...
- Introduced [lilya.contrib.mail](./contrib/mail.mdx) providing a full-featured, async-first email framework.
...
- New [FormController](./templates.mdx#formcontroller) class based view template that is agnostic to any validation library.
...
- New [send_file](./contrib/files/send-file.mdx) to **contrib**.
- New [jsonify](./contrib/files/jsonify.mdx) to **contrib**.
...
- New dynamic, native and fast custom [serializers](./serializers.mdx).

### Changed

- To make Lilya cleaner in the installation we have now separated the installation. The [Lilya native client](./lilya-cli.mdx)
...
- Support for automatic cast call for a function is a function is provided to cast in the `loader` of the [Environment](./environments.mdx).
...
- Support for native [cache](./caching.mdx) with default to `InMemory`.
...
- Possibility and alternative way of doing the [dependencies](./dependencies.mdx) without the need of using the `Provides()`.
...
- [Resolve](./dependencies.mdx#the-resolve-dependency-object) dependency injection functionality. This resolve acts with the `Provider` and
...
You need to also pass a `enable_intercept_global_exceptions` in the `Lilya` instance or [settings](./settings.mdx) as this
...
- Added support for `redirect_exception` when using [WSGIMiddleware](./middleware.mdx#wsgimiddleware). This
flag when passed, you can add a `HTTPException` exception handler that will make Lilya capture the WSGI exceptions.
- Exception handlers on a global level can now be passed via [settings](./settings.mdx)
...
- Support for a second form of declaring [directives](./directives/directive-decorator.mdx).
...
Assuming you have the [encoders](./encoders.mdx) for Pydantic and Struct installed in your application (or any other) you
desire.

Lilya uses the internal Encoders to parse and transform them properly.

:::note
    `infer_body` is set to `False` by default but you can override it in the [settings](./settings.mdx).
:::
...
- Support for [OpenAPI](./openapi.mdx) documentation under the new `@openapi()` decorator. This is optional and can be used
...
- Support for [TemplateController](./templates.mdx#templatecontroller).
- Support for [ListController](./templates.mdx#listcontroller).
...
- New [LoggingConfig](./logging.mdx) for the logging configuration. This now allows you to setup
...
- [Observables](./observables.mdx) documentation section. This is a new feature that allows to create
...
- New [SessionContextMiddleware](./middleware.mdx#sessioncontextmiddleware) allowing to use the new `session` object
in a request context.

### Changed

- Updated the Context section by adding the [session context](./context.mdx#the-session-object) examples and explanation
how to use it.
...
- The [g](./context.mdx#the-g-object) definition and declaration.
...
- New [EnvironLoader](./environments.mdx) support.
...
* A native and **optional** [client](./lilya-cli.mdx).
