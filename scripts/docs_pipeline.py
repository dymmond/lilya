...
def _resolve_include_path(include_expr: str, source_file: Path, include_base_dir: Path) -> Path:
    """Resolve includes using the MkDocs `mdx_include` base-path behavior first."""
    candidates = [
...
def render_markdown_with_includes(content: str, source_file: Path, include_base_dir: Path) -> str:
