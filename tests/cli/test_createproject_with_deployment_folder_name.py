...
def test_create_project(create_folders):
    (o, e, ss) = run_cmd(
        "tests.cli.main:app",
        "lilya createproject myproject --with-structure --with-deployment --deployment-folder-name deploy",
    )
    assert ss == 0
...
def test_create_project_files_without_env_var_and_with_app_flag(create_folders):
    (o, e, ss) = run_cmd(
        "tests.cli.main:app",
        "lilya --app tests.cli.main:app createproject myproject --with-structure --with-deployment --deployment-folder-name deploy",
        is_app=False,
    )
    assert ss == 0
