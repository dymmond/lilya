...
async def listen_email_send() -> None:
    ...
    async with email_lock:
        TOTAL_EMAIL += 1
    logger.info("Listening to email send")
    email_event.set()
...
def listen_email_send_sync() -> None:
    ...
    global TOTAL_EMAIL
    TOTAL_EMAIL += 1
    logger.info("Listening to email send in sync mode")
    email_event.set()
...
async def listen_extra_event() -> None:
    ...
    async with extra_lock:
        TOTAL_EXTRA += 1
    logger.info("Listening to extra event")
    extra_event.set()
...
async def assign_default_roles(**kwargs: Any):
    ...
    logger.info(f"Assigning default roles to user ID {data['id']}...")
    role_event.set()
...
async def test_user_registration():
    ...
    with create_client(
        routes=[Path("/register", handler=register_user, methods=["POST"])]
    ) as client:
