...
from lilya.dependencies import Depends, PureScope, inject
...
async def test_depends_async_callable_with_dependencies_map():
    ...
    result = await dependency.resolve(dependencies_map={"a": provide_a, "b": 3})
    assert result == 12
...
def test_use_cache_true_caches_per_instance():
    ...
    assert value1 == 1
    assert value2 == 1
    assert state["count"] == 1  # only once
...
def test_unresolvable_parameter_raises():
    ...
    assert "Could not resolve parameter 'a'" in str(exc.value)
...
async def test_scope_keeps_session_open_until_exit():
    ...
    # The generator is exhausted immediately in requestless mode
    assert sess.closed


async def test_depends_scope_with_pure_scope_cleanup():
    flag = {"closed": False}
    gen = make_sync_gen(flag)
    dep = Depends(gen)

    async with PureScope() as scope:
        value = await dep.resolve(scope=scope)
        assert value == 99

        # The generator finalizes immediately after resolution
        assert flag["closed"]
...
async def test_async_dependency_with_cache_and_scope():
    fn, state = make_async_counter()
    dep = Depends(fn, use_cache=True)
    dep.scope = Scope.APP
