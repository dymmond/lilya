...
def test_path_converters_with_reverse(client):
    response = client.get("/int/5")
    assert response.status_code == 200
    assert response.json() == {"int": 5}
...
def test_make_absolute_url_for_with_reverse():
    assert (
        reverse(
            "homepage",
            app=app,
    ...
    assert (
        reverse(
            "homepage",
            app=app,
        ).make_absolute_url(base_url="https://example.org/root_path/")
        == "https://example.org/root_path/"
    )
    assert (
        reverse("user", app=app, path_params={"username": "lilya"}).make_absolute_url(
    ...
    async def __call__(self, scope: Scope, receive: Receive, send: Send) -> None:
        ...
        async def modified_send(msg: Message) -> None:
            if msg["type"] == "http.response.start":
                msg["headers"] = [*msg["headers"], (b"X-Test", b"Set by middleware")]
            await send(msg)
...
def test_mounted_middleware_does_not_catch_exception(
    ...
    class NamedDefineMiddleware:
        ...
        async def __call__(self, scope: Scope, receive: Receive, send: Send) -> None:
            async def modified_send(msg: Message) -> None:
                if msg["type"] == "http.response.start":
                    msg["headers"] = [*msg["headers"], (f"X-{self.name}".encode(), b"true")]
                await send(msg)
