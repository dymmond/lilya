...
async def test_lilya_integration_in_memory(memory_cache):
    """Ensure the caching decorator works in an Lilya application with in-memory caching."""

    app = Lilya()

    @app.get("/cached/{value}")
    @cache(backend=memory_cache, ttl=1)
    ...
    with TestClient(app) as client:
        ...
        cache_key = (
            "tests.caches.test_decorator.test_lilya_integration_in_memory.cached_endpoint:10"
        )
        time.sleep(1)
...
async def test_lilya_integration_default():
    """Ensure the caching decorator works in an Lilya application with default caching."""
    app = Lilya()
...
async def test_lilya_integration_in_redis(redis_cache):
    """Ensure the caching decorator works in an Lilya application in redis."""
    app = Lilya()
