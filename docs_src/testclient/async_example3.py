...
from lilya.testclient import create_async_client
...
async def test_router():
    async with create_async_client(routes=routes) as client:
        ...
        response = await client.get("/nested/test")
        assert response.status_code == 200
        assert response.text == "Hello, world"

        response = await client.get("/nested/another/test/fluid")
        assert response.status_code == 200
        assert response.text == "Hello, world"

        async with client.websocket_connect("/ws") as session:
