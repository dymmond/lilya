...
def setup_tracing(config: OpenTelemetryConfig | None = None) -> None:
    ...
    if trace is None:
        raise RuntimeError("OpenTelemetry packages not installed. Install 'opentelemetry-sdk' and an exporter.")
    ...
    if cfg.exporter == "console":
        ...
        if endpoint.endswith(":4317"):
            exporter = OTLPgRPCExporter(endpoint=endpoint, insecure=cfg.otlp_insecure)
        else:
